...
import { plantnetLimiter } from '@/shared/utils/rateLimit';
import { cropDoctorLog } from '@/shared/utils/logger';

// In dev: Vite proxies /api/plantnet/* → https://my-api.plantnet.org/v2/identify/*
// In prod: Vercel serverless function at /api/plantnet reads the API key server-side
// NOTE: API key is NOT sent from the browser — the proxy handles it
...
async function identifyWithPlantNet(file) {
  ...
  // API key is handled by the serverless proxy — NOT sent from the browser
  const res = await fetch(`${PLANTNET_URL}?lang=en&nb-results=3`, {
    method: 'POST',
    body: form,
  });
...
export async function analyzeCropImage(file) {
  ...
  if (!plantnetLimiter.consume()) {
    cropDoctorLog.warn('Rate limited — too many scan requests');
    return {
      name: 'Too Many Scans',
      nameHi: 'बहुत ज़्यादा स्कैन',
      crop: 'Please Wait ⏳',
      severity: 'None',
      severityColor: '#F59E0B',
      confidence: 0,
      desc: 'You are scanning too quickly. Please wait a moment before trying again. This limit protects the service for all farmers.',
      treatment: [
        'Wait 30-60 seconds before scanning again',
        'Ensure you are uploading a clear, focused photo',
        'One scan at a time gives the best results',
      ],
      prevention: 'Take your time — one clear photo gives better results than many blurry ones.',
      hindiVoice: 'Aap bahut jaldi scan kar rahe hain. Kripya thodi der rukein aur dobara try karein.',
    };
  }

  cropDoctorLog.info('Starting crop analysis', { fileName: file.name, size: file.size });
